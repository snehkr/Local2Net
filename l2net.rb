...
while true
    ...
    if arc.include?"arm"
        system("wget -q --show-progress https://github.com/SnehKr/files/raw/main/ngrok/ngrok-stable-linux-arm.zip -O ngrok.zip")
        system("unzip ngrok.zip && rm -rf ngrok.zip")
        break
    elsif arc.include?"aarch64"
        system("wget -q --show-progress https://github.com/SnehKr/files/raw/main/ngrok/ngrok-stable-linux-arm64.tgz -O ngrok.tgz")
        system("tar -zxf ngrok.tgz && rm -rf ngrok.tgz")
        break
    elsif arc.include?"x86_64"
        system("wget -q --show-progress https://github.com/SnehKr/files/raw/main/ngrok/ngrok-stable-linux-amd64.zip -O ngrok.zip")
        system("unzip ngrok.zip && rm -rf ngrok.zip")
        break
    else
        system("wget -q --show-progress https://github.com/SnehKr/files/raw/main/ngrok/ngrok-stable-linux-386.zip -O ngrok.zip")
        system("unzip ngrok.zip && rm -rf ngrok.zip")
        break
    end
end
...
system("clear")
puts logo
puts "\n\n#{red}[ToolName]  #{cyan}  :[Local2Net]
#{red}[Version]    #{cyan} :[1.0]
#{red}[Description]#{cyan} :[LocalHost Exposing tool]
#{red}[Author]     #{cyan} :[SnehKr]
#{red}[Github]     #{cyan} :[https://github.com/SnehKr] 
#{red}[Telegram]  #{cyan} :[https://t.me/SnehKr]
#{red}[Email]      #{cyan} :[Snehkr.official@gmail.com]"
print "\n#{ask}#{yellow}---> #{green}"
about=gets.chomp
when "0"
    exit
